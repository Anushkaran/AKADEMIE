                                    ...
                                    <div class="modal fade text-left modal-info" id="tasks" tabindex="-1" role="dialog" aria-labelledby="myModalLabel110" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered" role="document">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <form action="{{route('partner.evaluations.sessions.tasks.attach',['evaluation' => $session->evaluation_id , 'session' => $session->id])}}" id="attach-tasks-form" method="post">
                                                        @csrf
                                                        <div class="form-group">
    ...
    <div class="modal modal-slide-in fade" id="modals-slide-in">
        <div class="modal-dialog sidebar-sm">
            <form class="add-new-record modal-content pt-0" method="post"
                  action="{{route('partner.evaluations.sessions.update',['evaluation' => $session->evaluation_id,'session' =>$session->id])}}">
                @csrf
                @method('PUT')
        ...
        const detachTask = id => {
            Swal.fire({
                ...
                if (result.value) {
                    let f = document.createElement("form");
                    f.setAttribute('method',"post");
                    f.setAttribute('action',`/partner/evaluations/{{$session->evaluation_id}}/sessions/{{$session->id}}/tasks/${id}`);
                    let i1 = document.createElement("input"); //input element, text
                    i1.setAttribute('type',"hidden");
                    i1.setAttribute('name','_token');
                    i1.setAttribute('value','{{csrf_token()}}');
